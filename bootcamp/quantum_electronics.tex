...
\begin{center}
\begin{figure}[h]
	\includegraphics[width=4cm]{fig/RC1.png}
	
	\includegraphics[width=5cm]{fig/keysightLP_bb.jpg}
\end{figure}
\end{center}
\end{column}
\begin{column}{6cm}
\begin{itemize}
\item Build a low pass filter as shown
\item Set the waveform generator to:
\begin{itemize}
\item Sine Wave
\item 5V
\item Sweep 100 Hz to 100MHz
\end{itemize}
\item Record the input, output attenuation, and phase
\end{itemize}
\end{column}
\end{columns}
\end{frame}
...
\begin{center}
	\includegraphics[width=12cm]{fig/Bode_RC1.jpg}
	
\vspace{0.1cm}	
	
$ f_{c} = \frac{1}{2 \pi RC} = \frac{1}{2 \pi (500) (01.* 10^{-6})} = 3.34 kHz $
\end{center}
\end{frame}
...
\begin{frame}\frametitle{Assignment: RLC Oscillator}
\begin{columns}
\begin{column}{4cm}
\begin{center}
\begin{figure}[h]
	\includegraphics[width=4cm]{fig/RLC2.png}
\end{figure}
\end{center}
\end{column}
\begin{column}{6cm}
\begin{itemize}
\item Build the oscillator as shown
\item Set the waveform generator to:
\begin{itemize}
\item Square Wave
\item 5V
\item Frequency = 100 Hz
\item Duty Cycle to 0.1%
\end{itemize}
\item Calculate the period of the oscillations.
\item Calculate the dampening over time. 
\end{itemize}
\end{column}
\end{columns}
\end{frame}

\section{Switches and Relays}
...
\begin{frame}\frametitle{Keysight Oscilloscope}
\begin{center}	
	\includegraphics[width=12cm]{fig/keysightoscope.jpg}
\end{center}
\end{frame}

\begin{frame}\frametitle{Keysight Waveform Generator}
\begin{center}	
	\includegraphics[width=12cm]{fig/keysightwave.jpg}
\end{center}
\end{frame}

\begin{frame}\frametitle{Keysight 3-Channel Power Supply}
\begin{center}	
	\includegraphics[width=12cm]{fig/keysightpower.jpg}
\end{center}
\end{frame}

\begin{frame}\frametitle{Assignment: Oscilloscope Basics}
\begin{columns}
\begin{column}{4cm}
\begin{center}	
	\includegraphics[width=4cm]{fig/keysightwave.jpg}
	
\vspace{1cm}	
	
	\includegraphics[width=4cm]{fig/keysightoscope.jpg}
\end{center}
\end{column}
\begin{column}{7.3cm}
\begin{itemize}
\item Connect Waverform Generator Output 1 to Oscilloscope Input 1
\item On the Waveform Generator: 
\begin{itemize}
\item Select waveform $\rightarrow$ Sine
\item Set amplitude to 5V
\item Set frequency to 10 kHz
\item Turn on Channel 1
\end{itemize}
\item On the Oscilloscope:
\begin{itemize}
\item Select Channel 1
\item Adjust vertical and horizontal until you see a standing sine wave
\end{itemize}
\item Experiment with adjusting amplitude, frequency, and waveform type
\item Note how the data in upper left, upper right, and bottom of oscope changes
\end{itemize}
\end{column}
\end{columns}
\end{frame}
...
\begin{frame}\frametitle{Assignment: Low Pass Filter}
\begin{columns}
\begin{column}{5cm}
\begin{center}	
	\includegraphics[width=5cm]{fig/keysightLP_bb.jpg}
...
\end{column}
\begin{column}{6cm}
Select sine and sweep on the waveform generator
\begin{itemize}
\item Amplitude 500mV
\item Sweep On
\item Log
\item Frequency Start 100Hz
\item Frequency Stop 100kHz
\item Sweep Time 30 seconds
\item With oscilloscope estimate change in gain and phase
\end{itemize}
\end{column}
\end{columns}
\end{frame}
...
\begin{frame}\frametitle{Assignment: Amplifying BandPass Filter}
\begin{columns}
\begin{column}{4cm}
\begin{center}	
	\includegraphics[width=3cm]{fig/ugly741_bb.jpg}
	\includegraphics[width=3.6cm]{fig/bandpassAmp_schem.jpg}
\end{center}
\end{column}
\begin{column}{8.0cm}
\begin{itemize}
\item Start without the $C_f$ capacitor and with shorting the $C_{in}$ capacitor. 
\item Using a $500 mV_{pp}$ input sinewave, you should see $10 V_{pp}$ inverted sinewave as the output.
\item Install $C_f$ and remove the jumper that is shorting $C_{in}$. 
\item Repeat the sweep from the Lowpass Filter Assignment
\end{itemize}
\begin{center}	
	\includegraphics[width=3cm,angle=90]{fig/LM741.jpg}
\end{center}
\end{column}
\end{columns}
\end{frame}
...
\begin{column}{4cm}
\begin{center}	
	\includegraphics[width=3cm]{fig/bandpassAmp_bb.jpg}
	\includegraphics[width=3.6cm]{fig/bandpassAmp_schem.jpg}
\end{center}
\end{column}
\begin{column}{8.0cm}
...
\section{Frequency Generators and Amplifiers}

\begin{frame}\frametitle{Frequency and Power Needs}

To laser cool, the light needs to be modulated. The modulators will be driven by a signal of a certain frequency and power:

\begin{center}	
	\includegraphics[width=10cm]{fig/aomeom.jpg}
\end{center}
\end{frame}


\begin{frame}\frametitle{Frequency Generators and Amplifiers}
\begin{columns}
\begin{column}{5cm}
\begin{center}	
	\includegraphics[width=5cm]{fig/rigol.jpg}
\end{center}
\end{column}
\begin{column}{5cm}
\begin{center}	
	\includegraphics[width=5cm]{fig/zx60.jpg}
\end{center}
\end{column}
\end{columns}
\end{frame}
...
\begin{frame}\frametitle{Recall Logarithms}
\begin{columns}
\begin{column}{7cm}
\begin{center}	
	\includegraphics[width=7cm]{fig/log10.jpg}
\end{center}
\end{column}
\begin{column}{4cm}
The logarithm of a number (in base 10) is the exponent to which 10 must be raised to produce the number.
\end{column}
\end{columns}
\begin{itemize}
\item If $x = 10^y$, then $y = log_{10}(x)$
\item With logarithms, multiplication becomes addition: $log_{10}(a \cdot b) = log_{10}(a) + log_{10}(b)$
\end{itemize}

\end{frame}

\begin{frame}\frametitle{Why Logarithms}

Logarithms are useful when working across many orders of magnitude:

\begin{center}	
	\includegraphics[width=10cm]{fig/whylog.png}
\end{center}
\end{frame}

\begin{frame}\frametitle{Why Logarithms}

Logarithms started being used in RF communication due to the inverse square law

\begin{center}	
	\includegraphics[width=7cm]{fig/isl.jpg}
\end{center}

\begin{itemize}
\item Surface Area of a Sphere: $A = 4 \pi r^2$
\item Intensity: $I(r) = \frac{I(1)}{r^2}$
\end{itemize}
\end{frame}

\begin{frame}\frametitle{Decibels (dB)}
\begin{columns}
\begin{column}{6cm}
\begin{itemize}
\item A decibel (dB) is an expression of a ratio
\item dB = $10 * log_{10}(\frac{a}{b})$
\item Decibels being a ratio means that they are relative to some reference
\item Sound dB (or dB Audio or dBA) expresses the ratio of a sound relative to the threshold of human hearing
\end{itemize}
\end{column}
\begin{column}{5cm}
\begin{center}	
	\includegraphics[width=5cm]{fig/dba.jpg}
\end{center}
\end{column}
\end{columns}
\end{frame}

\begin{frame}\frametitle{Power Measurements (dBm)}
\begin{columns}
\begin{column}{6cm}
\begin{itemize}
\item To measure absolute power in dB, a reference needs to be defined
\item It is useful to set the reference to 1 mW:
\[1 mW \rightarrow 0 dBm\]
\item The "m" in dBm indicating the reference is 1 mW.
\end{itemize}
\end{column}
\begin{column}{5cm}
\begin{center}	
	\includegraphics[width=5cm]{fig/dBm_chart.jpg}
\end{center}
\end{column}
\end{columns}
\end{frame}

\begin{frame}\frametitle{Using dB}
When using dB, gains and losses (attenuation) are additive.

\begin{itemize}
\item Recall: $log_{10}(a \cdot b) = log_{10}(a) + log_{10}(b)$
\item Also: $log_{10}(a / b) = log_{10}(a) - log_{10}(b)$
\item This leads to: "initial power" + 10 dB = 10 times the initial power
\item Examples \footnote{Note that 3dB = doubling}:
	\begin{itemize}
	\item $0 dBm + 10 dB = 10 dBm = 10 mW$
	\item $0 dBm + 30 dB = 30 dBm = 1000 mW = 1 W$
	\item $10 dBm + 3 dB = 13 dBm = 20 mW$
	\item $20 dBm - 3 dB = 17 dBm = 50 mW$
	\end{itemize}
\item Another Example: How much is 33 dBm?
	\begin{itemize}
	\item $33dBm$ = $33 dB $ relative to $ 1 dBm$
	\item $33dB = 10 dB + 10 dB + 10 dB + 3 dB$
	\item $33dBm = 10 * 10 * 10 * 2 * 1mW$
	\item $33dBm = 2000 mW $ or $ 2 W$
	\end{itemize}
\end{itemize}
...
\begin{frame}\frametitle{Gooch \& Housego 3200-124 AOM}
\begin{columns}
\begin{column}{5.5cm}
\begin{center}	
	\includegraphics[width=5.5cm]{fig/gh3200_124spec.jpg}
\end{center}
\end{column}
\begin{column}{5.5cm}
\begin{center}	
	\includegraphics[width=5.5cm]{fig/zhlspec.jpg}
\end{center}
\end{column}
\end{columns}
\end{frame}

\begin{frame}\frametitle{Gooch \& Housego 3080-122 AOM}
\begin{columns}
\begin{column}{5.5cm}
\begin{center}	
	\includegraphics[width=5.5cm]{fig/gh3080_122spec.jpg}
\end{center}
\end{column}
\begin{column}{5.5cm}
\begin{center}	
	\includegraphics[width=5.5cm]{fig/zx60spec.jpg}
\end{center}
\end{column}
\end{columns}
\end{frame}

\begin{frame}\frametitle{MOT Modulation Scheme}
\
\begin{center}	
	\includegraphics[width=8.5cm]{fig/motamps.jpg}
\end{center}
