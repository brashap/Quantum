...
\begin{column}{6cm}
\begin{center}
\includegraphics[width=6cm]{fig/profile.png}
\end{center}
\end{column}
\begin{column}{5cm}
The laser beam has a Gaussian profile.
\begin{itemize}
\item Setup a telescope to expand the beam size
\item Focus on the beam on an intensity detector
\item Moving the knife edge across the beam path, record the drop in intensity vs distance.
